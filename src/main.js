  ...
  border-radius: 2px; 
    ...
    getCompletions(editor, session, pos, prefix, callback) {
        ...
        if (
            ...
            matchedSnippets[0].prefix !== lastWord
        ) {
            const fileName = activeFile.filename
                ...
                .split(".")
                .slice(0, -1)
                .join("."); // Obtém o nome do arquivo sem extensão
            ...
            if (
                ...
                suggestions.forEach(
                    suggestion =>
                        (suggestion.icon =
                            "icon ace_completion-icon ace_snippet")
                );
            }
    ...
    async setupImportReactComponent(tagName, directory) {
        try {
            ...
            const importRegex = new RegExp(
                `import\\s+${componentName}\\s+from\\s+'${relativePathWithoutExtension}'`
            );
