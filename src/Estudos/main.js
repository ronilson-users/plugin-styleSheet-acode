...
.ace_tooltip.ace_doc-tooltip {
 ...
 getCompletions(editor, session, pos, prefix, callback) {
  ...
  const matchedSnippets = snippets.filter(snippet =>
   snippet.prefix.startsWith(lastWord)
  );
  ...
  if (matchedSnippets.length > 0 && matchedSnippets[0].prefix !== lastWord) {
   ...
   const suggestions = matchedSnippets.map(snippet => ({
    ...
    snippet: snippet.snippet.replace(/FILE_NAME/g, componentName),
    meta: snippet.type,
    value: snippet.snippet.replace(/FILE_NAME/g, componentName),
    type: "snippet",
    score: 600 || "",
   ...
   if (typeof extraSyntaxHighlightsInstalled !== "undefined" && extraSyntaxHighlightsInstalled) {
    suggestions.forEach(
     suggestion => (suggestion.icon = "icon ace_completion-icon ace_snippet")
    );
   }
 ...
 async setupImportReactComponent(tagName, directory) {
  try {
   const relativePath = this.calculateRelativePath(
    directory.directoryForCurrentFile,
    directory.directoryForTagName
   );
   ...
   const componentNameWithExtension = directory.directoryForTagName
    .split("/")
    .pop();

   console.log('componentNameWithExtension', componentNameWithExtension);

   const componentName = componentNameWithExtension
    .split(".")
    .slice(0, -1)
    .join(".")
    .replace(/^_/, '')
   ...
   const relativePathWithoutExtension = relativePath
    .split(".")
    .slice(0, -1)
    .join(".");
   ...
   const importRegex = new RegExp(
    `import\\s+${componentCase}\\s+from\\s+'${relativePathWithoutExtension}'`
   );
   ...
   if (!importRegex.test(code)) {
    ...
    window.toast(
     "The import was created at the top ☝️ successfully",
     3000
    );
   }
  } catch (error) {
 ...
 async componentLocation(tagName) {
  try {
   ...
   return {
    ...
    directoryForCurrentFile: currentFilePath
...
console.log(componentCase)
 ...
 calculateRelativePath(currentDirectory, targetDirectory) {
  ...
  while (
   ...
   currentPathParts[commonPathLength] ===
   targetPathParts[commonPathLength]
  ) {
   commonPathLength++;
  }
  ...
  if (
   commonPathLength === currentPathParts.length - 1 &&
   commonPathLength === targetPathParts.length - 1
  ) {
   relativePath = "./" + targetPathParts[targetPathParts.length - 1];
  } else {
   for (
    let i = commonPathLength;
